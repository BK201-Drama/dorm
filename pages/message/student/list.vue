	...
	<view>
		<view class="uni-header">
			<view class="uni-group hide-on-phone">
				<view class="uni-title">学生信息管理</view>
		...
		<view class="uni-container">
			<unicloud-db ref="udb" @load="onqueryload" collection="student,uni-id-roles" :options="options"
				:where="where" field="_id,studentID,name,gender,role{role_id,role_name},degree,dormBuildingNo,dormRoomNo,status"
				...
				<uni-table :loading="loading" :emptyText="error.message || '没有更多数据'" border stripe type="selection"
					...
					<uni-tr>
						...
						<uni-th align="center">入住状态</uni-th>	
...
<script>
	...
	const dbSearchFields = ["studentID","name","degree","dormBuildingNo","dormRoomNo"] // 支持模糊搜索的字段列表
	// 分页配置
	const pageSize = 20
	const pageCurrent = 1
		...
		methods: {
			...
			confirmDelete(item) {
				...
				uniCloud.callFunction({
					name:"dec_dormRoomNumber",
					data:{
						dormBuildingNo:item.dormBuildingNo,
						dormRoomNo:item.dormRoomNo
					},
					success:(res)=>{
						that.$refs.udb.remove(item._id)
						uniCloud.callFunction({
							name:'deleteRole',
							data:{
