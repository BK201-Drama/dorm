	...
	<view class="uni-container">
		<uni-forms ref="form" v-model="formData" :rules="rules" validateTrigger="bind" @submit="submit">
			...
			<uni-forms-item name="role" label="角色列表">
				<uni-data-checkbox :localdata="roles" v-model="role"/>
			</uni-forms-item>
...
<script>
	...
	export default {
		...
		methods: {
			 ...
			 */
			submitForm() {
				this.formData.role[0] = this.role;
			...
			submit(event) {
				...
				event.detail.value.role = this.formData.role
				...
				this.$request('system/user/addUser', value)
				    .then(res => {
						...
						if(that.formData.role.indexOf("dormManger")>-1){
							var data ={"name": "","dormMangerID": that.formData.username,"gender": 0,"role": ["dormManger"],"mobile": "", "dormBuildingNo": ""}
							const dbCollectionName = 'dormManger';
							db.collection('dormManger').add(data).then((res)=>{		
								...
								uni.showModal({
									...
									success: (res) => {
										if(res.confirm){
											uni.navigateTo({
												url:"../../message/dormManger/edit?name=" + that.formData.username +"&id="+ id
											})										
										}
									}
								})
						...
						else if(that.formData.role.indexOf("student")>-1){
							var data ={"name": "","studentID": that.formData.username,"gender": 0,"role": ["student"],"status": 1, "degree": "", "dormBuildingNo": "","dormRoomNo":"","applyStatus":3}
							const dbCollectionName = 'student';
							db.collection('student').add(data).then((res)=>{		
								var id = res.result.id
								uni.showModal({
									title:"提示",
									"content":"是否完善学生信息",
									success: (res) => {
										if(res.confirm){
											uni.navigateTo({
												url:"../../message/student/edit?name=" + that.formData.username +"&id="+ id
											})										
										}
									}
								})
			...
			loadroles() {
				db.collection('uni-id-roles').limit(10).get().then(res => {
					const roleIds = []
