	...
	<view>
		...
		<view class="uni-container">
			<unicloud-db ref="udb" @load="onqueryload" collection="uni-id-users,uni-id-roles" :options="options"
				:where="where" field="_id,username,role{role_id,role_name},status,register_date"
...
<script>
	...
	export default {
		...
		async confirmDelete(item) {
				...
				if(item.role =="宿管"){
					const res = await db.collection('dormManger').where({
					  'dormMangerID': item.username
					}).get()
					// console.log(res.result)		
					db.collection('dormManger').doc(res.result.data[0]._id).remove().then(res=>{
						this.$refs.udb.remove(item._id,{success:res=>{
							uni.showToast({								
								title: '删除成功',							
							})
						}})
					...
					uniCloud.callFunction({
						...
						data:{
							dormBuildingNo:res.dormBuildingNo,
							dormRoomNo:res.dormRoomNo
						},
						success:(res)=>{
							db.collection('student').doc(res.result.data[0]._id).remove().then(res=>{
								this.$refs.udb.remove(item._id,{success:res=>{
									uni.showToast({								
										title: '删除成功',							
									})
