	...
	<view class="login-box">
		<view class="login">
			<view class="uni-header no-padding">
				<view class="uni-title">宿舍管理系统登录</view>
			</view>
				...
				<uni-forms ref="form" v-model="formData" :rules="rules" @submit="submit">
					<uni-forms-item label="账号" name="username">
						<input ref="usernameInput" @confirm="submitForm" class="uni-input-border"  type="text"
							placeholder="请输入账号" v-model="formData.username" />
					</uni-forms-item>
					<uni-forms-item label="密码" class="icon-container" name="password">
		...
		methods: {
			...
			getDormBuildingNo(){				
					uni.getStorage({
						key: 'lastUsername',
						success: (res) => {
						const data = db.collection("dormManger").where({dormMangerID:res.data}).field({"dormBuildingNo":true}).get();
						data.then(res =>{
							uni.setStorage({
								key:"dormBuildingNo",
								data:res.result.data[0].dormBuildingNo
							})
							// console.log(res.result.data[0].dormBuildingNo);
						})
					}
				})								
			},
					...
					if (res.needCaptcha) {
						...
						return this.init().then((res) => {
							...
							uni.getStorage({
								...
								success:(res)=>{
									role =res.data
									if(res.data == "dormManger"){
										that.getDormBuildingNo();
									}
								}
							})
							if(role == "student"){
								uni.redirectTo({
									url:"../message/studentPage/studentPage"
								})
							}else{
							uni.redirectTo({
								url: this.indexPage,
